...
open class SyntaxRewriter {
  ...
  open func visit(_ node: LayoutConstraintSyntax) -> Syntax {
    return visitChildren(node) 
  }
  ...
  public func visit(_ node: Syntax) -> Syntax {
    ...
    case .genericRequirement: return visit(node as! GenericRequirementSyntax)
    ...
    case .layoutRequirement: return visit(node as! LayoutRequirementSyntax)
    case .layoutConstraint: return visit(node as! LayoutConstraintSyntax)
...
public protocol SyntaxVisitor {
  ...
  ///   - Parameter node: the node we are visiting.
  ///   - Returns: how should we continue visiting.
  mutating func visit(_ node: LayoutRequirementSyntax) -> SyntaxVisitorContinueKind

  /// The function called after visiting `LayoutRequirementSyntax` and its descendents.
  ///   - node: the node we just finished visiting.
  mutating func visitPost(_ node: LayoutRequirementSyntax)
  /// Visiting `LayoutConstraintSyntax` specifically.
  ///   - Parameter node: the node we are visiting.
  ///   - Returns: how should we continue visiting.
  mutating func visit(_ node: LayoutConstraintSyntax) -> SyntaxVisitorContinueKind

  /// The function called after visiting `LayoutConstraintSyntax` and its descendents.
  ///   - node: the node we just finished visiting.
  mutating func visitPost(_ node: LayoutConstraintSyntax)
...
open class SyntaxVisitorBase: SyntaxVisitor {
  ...
  open func visit(_ node: LayoutConstraintSyntax) -> SyntaxVisitorContinueKind {
    return .visitChildren
  }
  open func visitPost(_ node: LayoutConstraintSyntax) {}
...
fileprivate func doVisit<Visitor>(
  ...
  case .layoutConstraint:
    let node = LayoutConstraintSyntax(data)
    let needsChildren = visitor.visit(node) == .visitChildren
    // Avoid casting to `_SyntaxBase` if we don't need to visit children.
    if needsChildren && data.raw.numberOfChildren > 0 {
      visitChildren(data, parent: node, &visitor)
