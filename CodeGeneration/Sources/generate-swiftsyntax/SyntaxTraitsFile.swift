...
let syntaxTraitsFile = SourceFile {
  for trait in TRAITS {
    ProtocolDecl("""
      ...
      for child in trait.children {
        VariableDecl("var \(raw: child.swiftName): \(raw: child.typeName)\(raw: child.isOptional ? "?" : "") { get }")
        FunctionDecl("func with\(raw: child.name)(_ newChild: \(raw: child.typeName)?) -> Self")
      }
    }
    
    ExtensionDecl("public extension SyntaxProtocol") {
      FunctionDecl("""
  ...
  for node in SYNTAX_NODES where !node.isBase && !node.traits.isEmpty {
    ExtensionDecl("extension \(raw: node.name): \(raw: node.traits.map { $0 + "Syntax" }.joined(separator: ", ")) {}")
  }
}
