...
/// 
///  - `text`: `<unknown>`
public struct SkippedDeclSyntax: DeclSyntaxProtocol, SyntaxHashable, _LeafDeclSyntaxNodeProtocol {
  public let _syntaxNode: Syntax
  
  public init?(_ node: some SyntaxProtocol) {
    guard node.raw.kind == .skippedDecl else {
      return nil
    }
    self._syntaxNode = node._syntaxNode
  }
  ...
  public init(
      leadingTrivia: Trivia? = nil,
      _ unexpectedBeforeText: UnexpectedNodesSyntax? = nil,
      text: TokenSyntax,
      _ unexpectedAfterText: UnexpectedNodesSyntax? = nil,
      trailingTrivia: Trivia? = nil
    
  ) {
    // Extend the lifetime of all parameters so their arenas don't get destroyed
    // before they can be added as children of the new arena.
    self = withExtendedLifetime((SyntaxArena(), (unexpectedBeforeText, text, unexpectedAfterText))) { (arena, _) in
      let layout: [RawSyntax?] = [unexpectedBeforeText?.raw, text.raw, unexpectedAfterText?.raw]
      let raw = RawSyntax.makeLayout(
        kind: SyntaxKind.skippedDecl,
        from: layout,
        arena: arena,
        leadingTrivia: leadingTrivia,
        trailingTrivia: trailingTrivia
  ...
  public var unexpectedBeforeText: UnexpectedNodesSyntax? {
    get {
      return Syntax(self).child(at: 0)?.cast(UnexpectedNodesSyntax.self)
    }
    set(value) {
      self = Syntax(self).replacingChild(at: 0, with: Syntax(value), arena: SyntaxArena()).cast(SkippedDeclSyntax.self)
    }
  }
  
  /// ### Tokens
  /// 
  /// For syntax trees generated by the parser, this is guaranteed to be `<unknown>`.
  public var text: TokenSyntax {
    get {
      return Syntax(self).child(at: 1)!.cast(TokenSyntax.self)
    }
    set(value) {
      self = Syntax(self).replacingChild(at: 1, with: Syntax(value), arena: SyntaxArena()).cast(SkippedDeclSyntax.self)
    }
  }
  
  public var unexpectedAfterText: UnexpectedNodesSyntax? {
    get {
      return Syntax(self).child(at: 2)?.cast(UnexpectedNodesSyntax.self)
    }
    set(value) {
      self = Syntax(self).replacingChild(at: 2, with: Syntax(value), arena: SyntaxArena()).cast(SkippedDeclSyntax.self)
    }
  }
  
  public static var structure: SyntaxNodeStructure {
    return .layout([\Self.unexpectedBeforeText, \Self.text, \Self.unexpectedAfterText])
  }
}
