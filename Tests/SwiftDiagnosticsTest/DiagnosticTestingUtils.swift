...
/// Asserts that the annotated source generated by `DiagnosticsFormatter` with diagnostics from parsing matches the expected output.
///
/// - Parameters:
///   - source: The source code string to parse.
///   - expectedAnnotatedOutput: The expected annotated source string that includes expected diagnostics formatting.
func assertParsedSource(
  _ source: String,
  expecting expectedAnnotatedOutput: String,
  function: StaticString = #function,
  file: StaticString = #file,
  line: UInt = #line
) {
  if areIntegrationTestsRecording && !isIntegrationTestsObserverActive {
    isIntegrationTestsObserverActive = true
    XCTestObservationCenter.shared.addTestObserver(IntegrationTestsObserver())
  }

  let tree = Parser.parse(source: source)
  let diagnostics = ParseDiagnosticsGenerator.diagnostics(for: tree)
  let annotatedSource = DiagnosticsFormatter.annotatedSource(tree: tree, diags: diagnostics, colorize: false)

  if areIntegrationTestsRecording && annotatedSource != expectedAnnotatedOutput {
    filesWithChanges[File(path: file), default: [:]]["\(function)"] = annotatedSource
  }

  assertStringsEqualWithDiff(annotatedSource, expectedAnnotatedOutput, file: file, line: line)
}

var areIntegrationTestsRecording = true  //false

private var filesWithChanges: [File: [String: String]] = [:]

private var isIntegrationTestsObserverActive = false

private final class IntegrationTestsObserver: NSObject, XCTestObservation {
...
private struct File: Hashable {
  ...
  // I don't like this solution.. but it should do the trick for our needs.
  private var interpolatedPath: String {
    "\(path)"
  }

  static func == (lhs: File, rhs: File) -> Bool {
    lhs.interpolatedPath == rhs.interpolatedPath
  }

  func hash(into hasher: inout Hasher) {
    hasher.combine(interpolatedPath)
  }
}

private func writeChanges() {
  defer { filesWithChanges.removeAll() }

  for (file, changes) in filesWithChanges {
    do {
      ...
      let sourceLocationConverter = SourceLocationConverter(fileName: filePath, tree: sourceFileSyntax)

      // Then update the source
      let integrationTestsRewriter = IntegrationTestsRewriter(changes: changes)
      let updatedSource = integrationTestsRewriter.visit(sourceFileSyntax).description
...
private final class IntegrationTestsRewriter: SyntaxRewriter {
  let changes: [String: String]
  var currentlyVisitedChange: String?
  ...
  override func visit(_ node: FunctionDeclSyntax) -> DeclSyntax {
    if let change = changes[node.name.text + "()"] {
      currentlyVisitedChange = change
    }
  ...
  override func visit(_ node: FunctionCallExprSyntax) -> ExprSyntax {
    ...
    guard let expectingArgumentIndex = arguments.firstIndex(where: { $0.label?.text == "expecting" }),
      let currentlyVisitedChange
    else {
      return super.visit(node)
    }
    var node = node
    ...
    let openingPounds = stringLiteral?.openingPounds?.trimmed

    var expectingArgument = arguments[expectingArgumentIndex]
    expectingArgument.expression = #"""
      \#(openingPounds)"""
      \#(raw: currentlyVisitedChange)
      """\#(openingPounds)
      """#
    expectingArgument.expression.leadingTrivia = .newline

    arguments[expectingArgumentIndex] = expectingArgument

    node.arguments = arguments

    self.currentlyVisitedChange = nil
