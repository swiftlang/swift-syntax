...
- The `SwiftSyntax` and `SwiftParser` modules were merged
  - Description: The types from the `SwiftParser` module were incorporated in the `SwiftSyntax` module. This reduces the number of imports clients need to add to use SwiftSyntax and allows further performance optimizations if the parser can benefit from whole module optimization with the definition of the syntax nodes.
  - Pull request: 
  - Migration steps: Change imports of SwiftParser to imports of SwiftSyntax.
