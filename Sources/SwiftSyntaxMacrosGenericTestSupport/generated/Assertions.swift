...
func assertNote(
  ...
  assertStringsEqualWithDiff(
    ...
    failureHandler: {
      failureHandler(TestFailureSpec(underlying: $0))
    }
  )
...
public func assertMacroExpansion(
  ...
  let failureLocation = TestFailureLocation(
    fileID: fileID,
    filePath: filePath,
    line: line,
    column: column
  ...
  if let expectedFixedSource = expectedFixedSource {
    let messages = applyFixIts ?? context.diagnostics.compactMap {
      $0.fixIts.first?.message.message
    }
...
public func assertMacroExpansion(
  ...
  let expandedSourceFile = await origSourceFile.expand(
    macroSpecs: macroSpecs,
    contextGenerator: contextGenerator,
    indentationWidth: indentationWidth
  )
