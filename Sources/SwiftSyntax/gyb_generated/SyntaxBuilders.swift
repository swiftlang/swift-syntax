...
public struct GenericParameterClauseSyntaxBuilder {
  ...
  public mutating func useObsoletedWhereClause(_ node: GenericWhereClauseSyntax) {
    let idx = GenericParameterClauseSyntax.Cursor.obsoletedWhereClause.rawValue
    layout[idx] = node.raw
  }
  ...
  internal mutating func buildData() -> SyntaxData {
    ...
    if (layout[3] == nil) {
      layout[3] = RawSyntax.missingToken(TokenKind.rightAngle)
    }
...
public struct LayoutRequirementSyntaxBuilder {
  ...
  public mutating func useLeftTypeIdentifier(_ node: TypeSyntax) {
    let idx = LayoutRequirementSyntax.Cursor.leftTypeIdentifier.rawValue
    layout[idx] = node.raw
  }
  ...
  public mutating func useLayoutConstraint(_ node: LayoutConstraintSyntax) {
  ...
  internal mutating func buildData() -> SyntaxData {
    ...
    if (layout[2] == nil) {
      layout[2] = RawSyntax.missing(SyntaxKind.layoutConstraint)
    }
...
public struct LayoutConstraintSyntaxBuilder {
  private var layout =
    Array<RawSyntax?>(repeating: nil, count: 6)
  ...
  public mutating func useName(_ node: TokenSyntax) {
    let idx = LayoutConstraintSyntax.Cursor.name.rawValue
    layout[idx] = node.raw
  }

  public mutating func useLeftParen(_ node: TokenSyntax) {
    let idx = LayoutConstraintSyntax.Cursor.leftParen.rawValue
    layout[idx] = node.raw
  }

  public mutating func useSize(_ node: TokenSyntax) {
    let idx = LayoutConstraintSyntax.Cursor.size.rawValue
    layout[idx] = node.raw
  }

  public mutating func useComma(_ node: TokenSyntax) {
    let idx = LayoutConstraintSyntax.Cursor.comma.rawValue
    layout[idx] = node.raw
  }

  public mutating func useAlignment(_ node: TokenSyntax) {
    let idx = LayoutConstraintSyntax.Cursor.alignment.rawValue
    layout[idx] = node.raw
  }

  public mutating func useRightParen(_ node: TokenSyntax) {
    let idx = LayoutConstraintSyntax.Cursor.rightParen.rawValue
    layout[idx] = node.raw
  }
  ...
  internal mutating func buildData() -> SyntaxData {
    ...
    return .forRoot(RawSyntax.createAndCalcLength(kind: .layoutConstraint,
      layout: layout, presence: .present))
  }
}

extension LayoutConstraintSyntax {
  /// Creates a `LayoutConstraintSyntax` using the provided build function.
  /// - Parameter:
  ///   - build: A closure that wil be invoked in order to initialize
  ///            the fields of the syntax node.
  ///            This closure is passed a `LayoutConstraintSyntaxBuilder` which you can use to
  ///            incrementally build the structure of the node.
  /// - Returns: A `LayoutConstraintSyntax` with all the fields populated in the builder
  ///            closure.
  public init(_ build: (inout LayoutConstraintSyntaxBuilder) -> Void) {
    var builder = LayoutConstraintSyntaxBuilder()
    build(&builder)
    let data = builder.buildData()
    self.init(data)
  }
