...
func syntaxRewriterFile(
  ...
  let functionEffectSpecifiers =
    if functionEffectSpecifiers.isEmpty {
      functionEffectSpecifiers
    } else {
      " ".appending(functionEffectSpecifiers)
    }
  let functionCallOperators =
    if functionCallOperators.isEmpty {
      functionCallOperators
    } else {
      functionCallOperators.appending(" ")
    }
    ...
    try! ClassDeclSyntax(
      ...
      DeclSyntax(
        ...
        private func visitImpl<NodeType: SyntaxProtocol>(
          ...
          node = if let newNode = \(raw: functionCallOperators)visitAny(origNode) {
            newNode
          } else {
            \(raw: functionCallOperators)Syntax(visit(origNode.cast(NodeType.self)))
          }
