...
fileprivate extension ChildKind {
  func hasSameType(as other: ChildKind) -> Bool {
    ...
    case (.collection(kind: let kind, _), .collection(kind: let otherKind, _)):
      return kind == otherKind
    case (.token(let choices, _, _), .token(let otherChoices, _, _)):
      return choices == otherChoices
    case (.node(let kind), .collection(kind: let otherKind, _)):
      return kind == otherKind
    case (.collection(kind: let kind, _), .node(let otherKind)):
      return kind == otherKind
    default:
      return false
    }
